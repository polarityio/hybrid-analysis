...
function doLookup(entities, options, cb) {
    ...
    async.parallelLimit(tasks, 10, (err, results) => {
        ...
        let score = options.minScore
        results.forEach(result => {
            let tScores = [];
            result.body.forEach(function(score){
              tScores.push(score.threat_score);
            });
            let tScore = tScores[0];

            if(result.body === null || _isMiss(result.body) || tScore < options.minScore){
                lookupResults.push({
